...
uint8_t spi_return[20];			// spi_return[0] reserved for null/meaningless values (values returned from write commands)
...
void MAX_init()
{
	...
	spi_return[0] = MAX_SPI_CMD(0x14);				// TOF1[15:8] – Pulse Launcher Size
	...
	spi_return[0] = MAX_SPI_CMD(0xC1);				// TOF2[15:13] – Stop Hits
	...
	spi_return[0] = MAX_SPI_CMD(0x08);				// TOF3[15:14] – Reserved
													// TOF3[13:8] – HIT1 Wave Select
	spi_return[0] = MAX_SPI_CMD(0x09);				// TOF3[7:6] – Reserved
	...
	spi_return[0] = MAX_SPI_CMD(0x0A);				// TOF4[15:14] – Reserved
													// TOF4[13:8] – HIT3 Wave Select
	spi_return[0] = MAX_SPI_CMD(0x0B);				// TOF4[7:6] – Reserved
	...
	spi_return[0] = MAX_SPI_CMD(0x0C);				// TOF5[15:14] – Reserved
													// TOF5[13:8] – HIT5 Wave Select
	spi_return[0] = MAX_SPI_CMD(0x0D);				// TOF5[7:6] – Reserved
	...
	spi_return[0] = MAX_SPI_CMD(0x06);				// TOF6[15:8] – Comparator Return Offset Upstream
	spi_return[0] = MAX_SPI_CMD(0x03);				// TOF6[7:0] – Comparator Offset Upstream
	DEASSERT_MAX_TDC_CE;

	ASSERT_MAX_TDC_CE;
	// TOF7 Register - comparator downstream
	spi_return[0] = MAX_SPI_CMD(WRITE_TOF7);		// Write to TOF7
	spi_return[0] = MAX_SPI_CMD(0x06);				// TOF7[15:8] – Comparator Return Offset Downstream
	spi_return[0] = MAX_SPI_CMD(0x03);				// TOF7[7:0] – Comparator Offset Downstream
	DEASSERT_MAX_TDC_CE;
	...
	spi_return[0] = MAX_SPI_CMD(0xE0);
	DEASSERT_MAX_TDC_CE;

	ASSERT_MAX_TDC_CE;
	spi_return[0] = MAX_SPI_CMD(WRITE_CLBRT_CTRL);				// Set Calibration and Control Register
	spi_return[0] = MAX_SPI_CMD(0x02);
	spi_return[0] = MAX_SPI_CMD(0x7F);
	DEASSERT_MAX_TDC_CE;
...
// TODO add capability to handle other even interrupts

	GPIO_IntDisable(0x0010);

	ASSERT_MAX_TDC_CE;
	spi_return[0] = MAX_SPI_CMD(READ_INT_STAT_REG);		// Read status register
	spi_return[1] = NULL_CMD;
	spi_return[2] = NULL_CMD;
	DEASSERT_MAX_TDC_CE;
	...
	if((0x10 & spi_return[1]) == 0x10)
	{
		...
		spi_return[4] = NULL_CMD;
		spi_return[5] = NULL_CMD;
		DEASSERT_MAX_TDC_CE;

		ASSERT_MAX_TDC_CE;
		spi_return[0] = MAX_SPI_CMD(TOF_DIFF_FRAC); 	// Read TOF_DIFFFrac
		spi_return[6] = NULL_CMD;
		spi_return[7] = NULL_CMD;
		DEASSERT_MAX_TDC_CE;
	}
